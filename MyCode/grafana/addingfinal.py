...
def insert_or_update_device_info(ip, device_name, date, udp_sent, udp_received, tcp_sent, tcp_received, icmp_sent, ic                                                                        mp_received, dns_queries, distinct_dns_queries):
    connection = connect_db()
    cursor = connection.cursor()
    cursor.execute("""
        INSERT INTO device_info (ip_address, device_name, date, udp_bytes_sent, udp_bytes_received, tcp_bytes_sent, t                                                                        cp_bytes_received, icmp_bytes_sent, icmp_bytes_received, dns_queries, distinct_dns_queries)
    ...
    """, (ip, device_name, date, udp_sent, udp_received, tcp_sent, tcp_received, icmp_sent, icmp_received, dns_querie                                                                        s, distinct_dns_queries))
    connection.commit()
    cursor.close()
    connection.close()
            ...
            if filename.endswith(".pcap"):
                        ...
                        for ip_addr in [ip, dst_ip]:
                            if ip_addr in device_dictionary:
                                ...
                                if date_key not in temp_traffic_data[ip_addr]:
                                    temp_traffic_data[ip_addr][date_key] = {'udp_sent': 0, 'udp_received': 0, 'tcp_se                                                                        nt': 0, 'tcp_received': 0, 'icmp_sent': 0, 'icmp_received': 0, 'dns_queries': 0, 'distinct_dns_queries': set()}
                ...
                for ip, date_data in temp_traffic_data.items():
                    ...
                    for date, data in date_data.items():
                        insert_or_update_device_info(ip, device_name, date, data['udp_sent'], data['udp_received'], d                                                                        ata['tcp_sent'], data['tcp_received'], data['icmp_sent'], data['icmp_received'], data['dns_queries'], len(data['disti                                                                        nct_dns_queries']))
...
def update_device_status(ip, device_name, current_time, last_seen_time):
    ...
    if existing_device:
        ...
        cursor.execute("UPDATE device_status SET previous_seen_time = %s, last_seen_time = %s WHERE id = %s", (previo                                                                        us_seen_time, last_seen_time, device_id))
    else:
        previous_seen_time = get_relative_time(last_seen_time)
        cursor.execute("INSERT INTO device_status (ip_address, device_name, last_seen_time, previous_seen_time) VALUE                                                                        S (%s, %s, %s, %s)", (ip, device_name, last_seen_time, previous_seen_time))
    connection.commit()
    cursor.close()
    connection.close()
...
def main():
    ...
    print("Updated device traffic data and DNS query information, including subfolders.")
